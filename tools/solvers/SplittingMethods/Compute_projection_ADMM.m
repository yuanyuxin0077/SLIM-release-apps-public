function [x,log]=Compute_projection_ADMM(x,A,funProj,options,R,ini_guess)
...
for i=1:maxit
    ...
    if adjust_rho==1
        [x,flag,relres,iter,resvec,lsres] = lsqr([I;sqrt(rho)*A],[y;sqrt(rho)*(z-u)],1e-7,1e3,[],[],x); %initial guess is the current x-estimate (warmstart)
        log.lsqr_it(i)=iter;
        log.lsqr_relres(i)=relres;
    ...
    if (r_pri_norm < log.eps_pri(i) && r_dual_norm < log.eps_dual(i))
        ...
        output_check_ADMM(Ax,funProj,x,feas_tol,r_pri_norm,i);
        break;
    end
...
output_check_ADMM(Ax,funProj,x,feas_tol,r_pri_norm,i);
%semilogy(log.r_pri_norm);pause(0.1)
end
%% Output checks

function []=output_check_ADMM(Ax,funProj,x,feas_tol,r_pri_norm,i)
